...
def mostrar_prediccion_dolar():
    ...
    if df is not None and not df.empty:
        ...
        fig_hist.update_layout(
            ...
            annotations=[
                dict(
                    x=pd.to_datetime("2025-04-13"),
                    y=df[df['Fecha'] == pd.to_datetime("2025-04-13")]['Promedio'].values[0] if not df[df['Fecha'] == pd.to_datetime("2025-04-13")].empty else df['Promedio'].max(),
